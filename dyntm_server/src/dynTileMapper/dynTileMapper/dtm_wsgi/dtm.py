...
PREFIX = ''
class DTMHandler(webapp.RequestHandler):
    def get(self):
        self._ts = None
        self._cl = None
        self._cs = None
...
class ClassifyHandler(DTMHandler):
    ...
    def real_get(self):
        ...
        if self.ts:
            ts = self.ts
            noform = self.request.get('noform')
            if noform:
    ...
    def real_post(self):
        ...
        """
        if self.ts:
            cl = self.request.get('classification')
            b64 = self.request.get('b64encode')
            if not cl:
...
class ColorSchemeHandler(DTMHandler):
    ...
    def real_post(self):
        ...
        if colors and name:
            ...
            c.n = len(color_list)
            c.colors = color_list
            alphas = self.request.get('alphas')
            if alphas:
                if alphas == "0,255,":
                    pass
                else:
...
class GMapHandler(DTMHandler):
    def real_get(self):
        ...
        cs_name = self.request.get('cs')
        if cs_name:
            values['colorScheme'] = cs_name
        cl_name = self.request.get('cl')
        if cl_name:
...
class QueryHandler(DTMHandler):
    def real_get(self):
        if not self.ts:
            return self.die(500)
        try:
            x,y = self.request.get('pixelxy').split(',')
            x,y = int(x),int(y)
            z = int(self.request.get('zoom').split(',')[0])
            tx,ty = x/256,y/256
            ox,oy = x%256,y%256
